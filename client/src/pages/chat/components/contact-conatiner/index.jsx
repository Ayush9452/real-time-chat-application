...
import { GET_ALL_CHANNEL_ROUTE, GET_CONTACT_FOR_DM_ROUTE } from "@/utils/constants";
...
const ContactsContainer = () => {
  ...
  useEffect(() => {
    const getContact = async () => {
      const res = await apiClient.get(GET_CONTACT_FOR_DM_ROUTE, {
    ...
    const getChannels = async () => {
      const res = await apiClient.get(GET_ALL_CHANNEL_ROUTE, { withCredentials: true });
  ...
  return (
    <div className="relative md:w-[35vw] lg:w-[30vw] xl:w-[20vw] bg-[#1b1c24] border-r-2 border-[#2f303b] w-full " >
      ...
      <div className="my-5">
        ...
        <div className="max-h-[38vh] overflow-y-auto no-scrollbar">
          <ContactList contacts={channels} isChannle={true} />
        </div>
      </div>
      <ProfileInfo />
    </div>
...
const Logo = () => {
  return (
    <div className="flex p-5  justify-start items-center gap-2">
      ...
      <span className="text-3xl font-semibold ">ChatMaster</span>
    </div>
  );
};
